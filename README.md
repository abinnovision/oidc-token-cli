...
plus a GitHub release. `.goreleaser.yaml` intentionally uses the `brews`
key (not `homebrew_casks` — casks get Gatekeeper-quarantined on an
unsigned binary); `goreleaser check` will report a `brews` deprecation
